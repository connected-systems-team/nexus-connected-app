...
export namespace PortCheckTool {
    ...
    export function parseOutput(output: ProcessOutput, host: string, portScanned: string): Result | undefined {
        ...
        for(const line of lines) {
            if(line.startsWith('Starting Nmap')) {
                ...
                const additionalIpsMatch = line.match(/Other addresses for .+ \(not scanned\): (.+)/);
                if(additionalIpsMatch && additionalIpsMatch[1]) {
                ...
                const [port, state, service] = parts;
                if(port) {
                ...
                const statsMatch = line.match(/Nmap done: (\d+) IP address(?:es)? \((\d+) host(?:s)? up\)/);
